...
public class player_four extends Applet {
   ...
   public void init() {
      ...
      setBackground(new Color(50,0,50));  // Dark green background.
      ...
      board.setBounds(20,20,564,564); // Note:  size MUST be 164-by-164 !
      board.newGameButton.setBounds(610, 60, 100, 30);
      board.resignButton.setBounds(610, 120, 100, 30);
      board.message.setBounds(0, 600, 330, 30);
   }
...
class CheckersCanvas extends Canvas implements ActionListener, MouseListener {
   ...
   public CheckersCanvas() {
      ...
      resignButton = new Button("End Game");
   ...
   void doNewGame() {
      ...
      currentPlayer = CheckersData.YELLOW;   // RED moves first.
      legalMoves = board.getLegalMoves(CheckersData.YELLOW);  // Get RED's legal moves.
   ...
   void doResign() {
       ...
       if (currentPlayer == CheckersData.RED)
          gameOver("Game ended  ");
       else if (currentPlayer == CheckersData.YELLOW)
           gameOver("Game ended  ");
       else if (currentPlayer == CheckersData.BLUE)
           gameOver("Game ended");
      ...
      for (int i = 0; i < legalMoves.length; i++)
         if (legalMoves[i].fromRow == row && legalMoves[i].fromCol == col) {
            ...
            {  message.setText("RED:  Make your move.");}
             ...
             if (currentPlayer == CheckersData.YELLOW)
                message.setText("YELLOW:  Make your move.");
             if (currentPlayer == CheckersData.BLUE)
   ...
   void doMakeMove(CheckersMove move) {
      ...
      */
      ...
      if (currentPlayer == CheckersData.RED) {
         ...
         if (legalMoves == null)
            gameOver("YELLOW has no moves");
         else if (legalMoves[0].isJump())
            message.setText("YELLOW:  Make your move.  You must jump.");
         else
            message.setText("YELLOW:  Make your move.");
      }
      else if (currentPlayer == CheckersData.YELLOW) {
          ...
          currentPlayer = CheckersData.BLUE;
          legalMoves = board.getLegalMoves(currentPlayer);
          if (legalMoves == null)
             gameOver("BLUE has no moves");
          else if (legalMoves[0].isJump())
   ...
   public void paint(Graphics g) {
      ...
      for (int row = 0; row < 14; row++) {
         for (int col = 0; col < 14; col++) {
             ...
             if(!((row<3 && col<3) || (col>10 && row<3)|| (row>10 && col<3) || (row>10 && col>10)))
             {
             ...
             switch (board.pieceAt(row,col)) {
                case CheckersData.RED:
                   ...
                   g.fillOval(4 + col*40, 4 + row*40, 32, 32);
                   break;
                ...
                case CheckersData.RED_KING:
                   ...
                   g.drawString("K", 7 + col*40, 16 + row*40);
                   break;
                ...
                case CheckersData.YELLOW:
                    ...
                    g.fillOval(4 + col*40, 4 + row*40, 32, 32);
                    break;
                 case CheckersData.BLUE:
                    g.setColor(Color.blue);
                    g.fillOval(4 + col*40, 4 + row*40, 32, 32);
                    break;
                 case CheckersData.YELLOW_KING:
                    g.setColor(Color.yellow);
                    g.fillOval(4 + col*40, 4 + row*40, 32, 32);
                    g.setColor(Color.black);
                    g.drawString("K", 7 + col*40, 16 + row*40);
                    break;
                 case CheckersData.BLUE_KING:
                    g.setColor(Color.blue);
      ...
      if (gameInProgress) {
         ...
         if (selectedRow >= 0) {
            ...
            g.drawRect(3 + selectedCol*40, 3 + selectedRow*40, 36, 36);
   ...
   */
   ...
   public void setUpGame() {
      ...
      for (int row = 0; row < 14; row++) {
         for (int col = 0; col < 14; col++) {
            if ( row % 2 == col % 2 ) {
            	 if(col>2 && col<11 && row <3)
                  board[row][col] = BLACK;
               else  if(col>2 && col<11 && row >10)
                  board[row][col] = RED;
               else  if(row>2 && row<11 && col<3)
            	   board[row][col] = YELLOW;
               else   if(row>2 && row<11 && col>10)
            	   board[row][col] = BLUE;
               else
                  board[row][col] = EMPTY;
            }
   ...
   public void makeMove(int fromRow, int fromCol, int toRow, int toCol) {
      ...
      if ((toRow == 0 || toCol == 13 || toCol == 0 ) && board[toRow][toCol] == RED)
         board[toRow][toCol] = RED_KING;
      if ((toRow == 13 || toCol == 13 || toCol == 0 ) && board[toRow][toCol] == BLACK)
         board[toRow][toCol] = BLACK_KING;
      if ((toRow == 0 || toCol == 13 || toRow == 13 ) && board[toRow][toCol] == YELLOW)
          board[toRow][toCol] = YELLOW_KING;
      if ((toRow == 0 || toCol == 0 || toRow == 13)  && board[toRow][toCol] == BLUE)
   ...
   public CheckersMove[] getLegalMoves(int player) {
      ...
      if (player != RED && player != YELLOW && player != BLACK   && player != BLUE )
         return null;
   ...
   private boolean canJump(int player, int r1, int c1, int r2, int c2, int r3, int c3) {
      ...
      if ( r3<0 || r3>13 || c3<0 || c3>13 || (r3<3&&c3<3) || (r3<3&&c3>10) || (r3>10&&c3<3) || (r3>10&&c3>10)   )
      ...
      if (player == RED) {
         ...
         else if (board[r2][c2] != BLACK && board[r2][c2] != BLACK_KING && board[r2][c2] != YELLOW && board[r2][c2] != YELLOW_KING && board[r2][c2] != BLUE && board[r2][c2] != BLUE_KING)
         { return false; } // There is no black piece to jump.
         else
         return true;  // The jump is legal.
      }
      else if (player == YELLOW) {
          if (board[r1][c1] == YELLOW && c3 < c1)
             return false;  // Regular red piece can only move  up.
          else if (board[r2][c2] != BLACK && board[r2][c2] != BLACK_KING && board[r2][c2] != RED && board[r2][c2] != RED_KING && board[r2][c2] != BLUE && board[r2][c2] != BLUE_KING)
             return false;  // There is no black piece to jump.
          else
          return true;  // The jump is legal.
       }
      else if (player == BLUE) {
          if (board[r1][c1] == BLUE && c1 < c3)
          { return false; } // Regular red piece can only move  up.
          else if (board[r2][c2] != BLACK && board[r2][c2] != BLACK_KING && board[r2][c2] != YELLOW && board[r2][c2] != YELLOW_KING && board[r2][c2] != RED && board[r2][c2] != RED_KING)
         ...
         else if (board[r2][c2] != RED && board[r2][c2] != RED_KING && board[r2][c2] != YELLOW && board[r2][c2] != YELLOW_KING && board[r2][c2] != BLUE && board[r2][c2] != BLUE_KING)
   ...
   private boolean canMove(int player, int r1, int c1, int r2, int c2) {
      ...
      if (r2 < 0 || r2 >= 14 || c2 < 0 || c2 >= 14 || (r2<3&&c2<3) || (r2<3&&c2>10) || (r2>10&&c2<3) || (r2>10&&c2>10)  )
      ...
      if (player == YELLOW) {
          if (board[r1][c1] == YELLOW && c1 > c2)
              return false;  // Regualr red piece can only move down.
           return true;  // The move is legal.
       }
      if (player == BLUE) {
          if (board[r1][c1] ==BLUE && c2 > c1)
              return false;  // Regualr red piece can only move down.
           return true;  // The move is legal.
       }
